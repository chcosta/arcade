...
namespace Microsoft.DotNet.Build.Tasks.NuGet
{
    ...
    public class PackNuSpec : Task
    {
        ...
        public override bool Execute()
        {
            ...
            {
                Log.LogError("Nuspec does not exist: " + File);
                return false;
            }
            ...
            try
            {
                Log.LogMessage($"Loading nuspec {File}");
                ...
                using (var file = System.IO.File.OpenRead(File))
                {
                    ...
                    if (!manifest.HasFilesNode)
                    {
                        ...
                        Log.LogWarning(
                            null, ArcadeTaskErrors.NuspecMissingFilesNode, null, File, 0, 0, 0, 0,
                ...
                packageBuilder = new PackageBuilder(File, packageBasePath, PropertyProvider, IncludeEmptyDirectories);

            }
            catch (InvalidDataException ex)
            {
                Log.LogError(
                    null, ArcadeTaskErrors.InvalidNuspecFile, null, File, 0, 0, 0, 0,
                    message: ex.Message);
                return false;
            }
        ...
        private void AddFiles(PackageBuilder builder)
        {
            foreach (var file in PackageFiles)
            {
                ...
                {
                    Log.LogError(
                        null, ArcadeTaskErrors.InvalidPackagePathMetadata, null, File, 0, 0, 0, 0,
